...
def printYieldTex(signal_name,xmass,len_x,m2sig,m1sig,exp,obs,up1sig,up2sig,m2sigerr,m1sigerr,experr,obserr,up1sigerr,up2sigerr):
    
    text_yields = open('yieldDir_hybrid/yields_{}.tex'.format(signal_name), "w")
    ...
    CL_horizontal = True
    if(CL_horizontal):
        tabEntry="|c|c|c|c|c|c"
        
        text_yields.write('\n \\centering')
        beginTab = '\n \\resizebox{1.1\\textwidth}{!}{\\begin{tabular}{ |c'+tabEntry+'| } '
        text_yields.write(beginTab)
        text_yields.write('\n \hline')

        tabMasses = ' TeV & '.join(str(m) for m in xmass)
        writeMasses = '\n Mass [TeV] & -2$\sigma$ & -1$\sigma$ & Median expected & Observed & +1$\sigma$ & +2$\sigma$ \\\\'
        text_yields.write(writeMasses)
        text_yields.write('\n \hline')
        for i in range(len(xmass)):
            forMass = '\n {} & {:.2E} $\pm$ {:.2E} & {:.2E} $\pm$ {:.2E} & {:.2E} $\pm$ {:.2E} & {:.2E} $\pm$ {:.2E} & {:.2E} $\pm$ {:.2E} & {:.2E} $\pm$ {:.2E} \\\\'.format(str(xmass[i]),m2sig[i],m2sigerr[i],m1sig[i],m1sigerr[i],exp[i],experr[i],obs[i],obserr[i],up1sig[i],up1sigerr[i],up2sig[i],up2sigerr[i])
            text_yields.write(forMass)
        text_yields.write('\hline')
        ...
        text_yields.close()
        '''
        command = 'pdflatex yieldDir_hybrid/yields_{}.tex'.format(signal_name)
        command2 = 'mv yields_{}.pdf yieldDir_hybrid/'.format(signal_name)
        command3 = 'rm yields_{}.log'.format(signal_name)
        command4 = 'rm yields_{}.aux'.format(signal_name)
        os.system(command)
        os.system(command2)
        os.system(command3)
        os.system(command4)
        ...
        text_yields.write(writeMasses)
        text_yields.write('\n \hline')
        tabm2sig = '\n -2$\sigma$ &' + ' & '.join(['{:.2e} $\pm$ {:.2e}'.format(m2sig[i], m2sigerr[i]) for i in range(len_x)]) +' \\\\'
        text_yields.write(tabm2sig)
        tabm1sig = '\n -1$\sigma$ &' + ' & '.join(['{:.2e} $\pm$ {:.2e}'.format(m1sig[i], m1sigerr[i]) for i in range(len_x)]) + ' \\\\'
        text_yields.write(tabm1sig)
        tabmexp = '\n Median expected &' + ' & '.join(['{:.2e} $\pm$ {:.2e}'.format(exp[i], experr[i]) for i in range(len_x)]) + ' \\\\'
        text_yields.write(tabmexp)
    
        tabobs = '\n Observed &' + ' & '.join(['{:.2e} $\pm$ {:.2e}'.format(obs[i], obserr[i]) for i in range(len_x)]) + ' \\\\'
        text_yields.write(tabobs)
    
        tabp1sig = '\n +1$\sigma$ &' + ' & '.join(['{:.2e} $\pm$ {:.2e}'.format(up1sig[i], up1sigerr[i]) for i in range(len_x)]) + ' \\\\'
        text_yields.write(tabp1sig)
    
        tabp2sig = '\n -2$\sigma$ &' + ' & '.join(['{:.2e} $\pm$ {:.2e}'.format(up2sig[i], up2sigerr[i]) for i in range(len_x)]) + ' \\\\ \hline'
        text_yields.write(tabp2sig)
...
y_limit_err = array('d')
y_mclimit_err = array('d')
y_mclimitlow68_err = array('d')
y_mclimitup68_err = array('d')
y_mclimitlow95_err = array('d')
y_mclimitup95_err = array('d')

tdrstyle.setTDRStyle()



iDir = '/opt/sbg/cms/ui3_data1/gcoulon/CMSSW_11_3_4/src/HSCPLimit/LimitComputation_MassSpectrum/limitTrees_SR3_Test_SystK1C1_codeChanged_CLS_merged/'




#THIS IS FOR BKG DIVIDED BY 2
#iDir = '/opt/sbg/cms/ui6_data1/rhaeberl/CMSSW_11_3_4/src/HSCPLimit/LimitComputation_MassSpectrum/limitTrees_SR3_test_bkg_div2/'
#THIS IS FOR LARGE WINDOWS

#iDir = '/opt/sbg/cms/ui6_data1/rhaeberl/CMSSW_11_3_4/src/HSCPLimit/LimitComputation_MassSpectrum/limitTrees_SR3_test_UnB_v4_Raph_largeWindows_withGoodSignals/'
# For each signal
for this_index, this_name in enumerate(signal_names):
    ...
    this_output = TFile.Open(iDir+'/higgsCombine.{}.HybridNew.all.mH120.root'.format(this_name))
    ...
    for ievent in range(int(this_tree.GetEntries())):
        ...
        #print("this_index = {}, this_name = {}, this_xsec = {}, this_mass = {}, ievent = {}".format(this_index,this_name,this_xsec,this_mass,ievent))
        #print("quantile Expected from root file : {} ".format(this_tree.quantileExpected))
        # Nominal expected
        
        if this_tree.quantileExpected == 0.5:
            y_mclimit.append(this_tree.limit*this_xsec)
            y_mclimit_err.append(this_tree.limitErr*this_xsec)
        ...
        if round(this_tree.quantileExpected,2) == 0.16:
            ...
            y_mclimitlow68_err.append(this_tree.limitErr*this_xsec)
        ...
        if round(this_tree.quantileExpected,2) == 0.84:
            ...
            y_mclimitup68_err.append(this_tree.limitErr*this_xsec)
        # -2 sigma expected
        if round(this_tree.quantileExpected,3) == 0.025:
            ...
            y_mclimitlow95_err.append(this_tree.limitErr*this_xsec)
        # +2 sigma expected
        if round(this_tree.quantileExpected,3) == 0.975:
            #print("{} has +2 sigma expected = {} -> limit*XS = {} * {}".format(this_name,this_tree.limit*this_xsec,this_tree.limit,this_xsec))
            y_mclimitup95.append(this_tree.limit*this_xsec)
            y_mclimitup95_err.append(this_tree.limitErr*this_xsec)
        ...
        if this_tree.quantileExpected == -1:
            if not options.blind:
                ...
                if not foundObs:
                    ...
                    y_limit_err.append(this_tree.limitErr*this_xsec)
                foundObs = True
...
if('DY' in signal_names[0]):
    nameSig = signal_names[0][:10]
if(not 'DY' in signal_names[0]):
    nameSig = re.findall(r'[^0-9]+(?=[0-9])', signal_names[0])[0]
if('ZPrimeSSM' in signal_names[0]):
    nameSig = 'ZprimeSSM'

#if('ZPrimeSSM' in signal_names[0]):
    

printYieldTex(nameSig,x_mass,len(x_mass),y_mclimitlow95,y_mclimitlow68,y_mclimit,y_limit,y_mclimitup68,y_mclimitup95,y_mclimitlow95_err,y_mclimitlow68_err,y_mclimit_err,y_limit_err,y_mclimitup68_err,y_mclimitup95_err)
...
if (debug > 0) : 
    ...
    print('len y_mclimit: {}'.format(len(y_mclimit)))
    print('len y_mclimit_err: {}'.format(len(y_mclimit_err)))
    print('y_mclimitlow95: {}'.format(y_mclimitlow95))
    print('y_mclimitlow95_err: {}'.format(y_mclimitlow95_err))
    print('y_mclimitlow68: {}'.format(y_mclimitlow68))
    print('y_mclimitlow68_err: {}'.format(y_mclimitlow68_err))
    print('y_mclimit: {}'.format(y_mclimit))
    print('y_mclimit_err: {}'.format(y_mclimit_err))
    print('y_mclimitup68: {}'.format(y_mclimitup68))
    print('y_mclimitup68_err: {}'.format(y_mclimitup68_err))
    print('y_mclimitup95: {}'.format(y_mclimitup95))
    print('y_mclimitup95_err: {}'.format(y_mclimitup95_err))


print(y_mclimit)
g_mclimit = TGraph(len(x_mass), x_mass, y_mclimit)
g_mclimit.SetTitle("median_expected_title")
g_mclimit.SetName("median_expected")
...
if not options.blind:
    ...
    #g_limit.SetTitle("Observed")
    g_limit.SetTitle("observed_title")
    g_limit.SetName("observed")
    ...
    g_mclimit.GetXaxis().SetTitle("m_{"+options.particle+"} [TeV]")  # NOT GENERIC
...
if not options.blind:
    g_limit.GetXaxis().SetTitle("m_{"+options.particle+"} [TeV]")  # NOT GENERIC
...
tmpyposition = 0.75 if options.blind else 0.71
...
tmpyposition = 0.66 if options.blind else 0.648
...
if 'tau' in cstr:
    ...
    text1.DrawLatex(expectedMassLimit-0.025,0, "  %0.2f TeV"%(expectedMassLimit))
...
if not options.blind:
    if 'tau' in cstr:
        ...
        text2.DrawLatex(obsMassLimit+0.05,0, "  %0.2f TeV"%(obsMassLimit))
    elif 'ZPrime' in cstr:
        text2.DrawLatex(obsMassLimit+0.09,0, "  %0.2f TeV"%(obsMassLimit))    
    else:
        text2.DrawLatex(obsMassLimit+0.05,0, "  %0.2f TeV"%(obsMassLimit))    
    #else:
        #text2.DrawLatex(obsMassLimit+0.065,0, "  %0.2f TeV"%(obsMassLimit))
...
climits.SaveAs(iDir+"limits_combine_"+options.lumi.replace('.','p')+"fb_"+options.signals[options.signals.find('/')+1:options.signals.find('.')]+'_'+cstr+".pdf")
climits.SaveAs(iDir+"limits_combine_"+options.lumi.replace('.','p')+"fb_"+options.signals[options.signals.find('/')+1:options.signals.find('.')]+'_'+cstr+".C")
climits.SaveAs(iDir+"limits_combine_"+options.lumi.replace('.','p')+"fb_"+options.signals[options.signals.find('/')+1:options.signals.find('.')]+'_'+cstr+".root")
