...
def get_unique_points(graph):
    points = set()
    for i in range(graph.GetN()):
        x = graph.GetX()[i]
        y = graph.GetY()[i]
        points.add((round(x, 10), round(y, 10)))
    return sorted(points)


def extract_expected_band(g_mclimit, g_error68, g_error95, output_txt):
    pts_pred = get_unique_points(g_mclimit)
    pts_68 = get_unique_points(g_error68)
    pts_95 = get_unique_points(g_error95)

    by_x = {}
    for x, y in pts_95:
        if x not in by_x:
            by_x[x] = {}
        by_x[x].setdefault('err95', []).append(y)
    for x, y in pts_68:
        if x not in by_x:
            by_x[x] = {}
        by_x[x].setdefault('err68', []).append(y)
    for x, y in pts_pred:
        if x not in by_x:
            by_x[x] = {}
        by_x[x]['pred'] = y

    print "Points written in " + output_txt

    with open(output_txt, 'w') as f:
        f.write("# x err95_inf err68_inf pred err68_sup err95_sup\n")
        for x in sorted(by_x):
            entry = by_x[x]
            pred = entry.get('pred', 0.0)
            err68 = sorted(entry.get('err68', []))
            err95 = sorted(entry.get('err95', []))
...
eta = "EtaMix"
iDir = '/opt/sbg/cms/safe1/cms/gcoulon/CMSSW_11_3_4/src/HSCPLimit/LimitComputation_MassSpectrum/MyDataCards/limit_8fp10_' + eta + '/'
endName = 'MyLimit.txt'

iDir = '/opt/sbg/cms/safe1/cms/gcoulon/CMSSW_11_3_4/src/HSCPLimit/LimitComputation_MassSpectrum/MyDataCards/limit_Original'
...
for this_index, this_name in enumerate(signal_names):
    ...
    this_output = TFile.Open(iDir+'/higgsCombine.{}.AsymptoticLimits.mH120.root'.format(this_name))
    ...
    for ievent in range(int(this_tree.GetEntries())):
        ...
        if this_tree.quantileExpected == 0.5:
            ...
            print("limit*XS = {} * {} = {}".format(this_tree.limit,this_xsec,this_tree.limit*this_xsec))
            #print("limit : {}".format(this_tree.limit))
            #print("XS : {}".format(this_xsec))
        # -1 sigma expected
...
extract_expected_band(g_mclimit, g_error, g_error95, iDir + endName)
...
if 'tau' in cstr:
    text1.DrawLatex(expectedMassLimit-0.005,0.00001, "  %0.2f TeV"%(expectedMassLimit))
elif 'tPrime' in cstr:
...
if not options.blind:
    if 'tau' in cstr:
        text2.DrawLatex(obsMassLimit+0.02,0.00001, "  %0.2f TeV"%(obsMassLimit))
    elif 'tPrime' in cstr:
