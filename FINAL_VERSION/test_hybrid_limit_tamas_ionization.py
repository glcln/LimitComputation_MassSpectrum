...
def printYieldTex(signal_name,xmass,len_x,m2sig,m1sig,exp,obs,up1sig,up2sig,m2sigerr,m1sigerr,experr,obserr,up1sigerr,up2sigerr):
    ...
    text_yields = open('yieldDir_hybrid_ionization/yields_{}.tex'.format(signal_name), "w")
    ...
    if(CL_horizontal):
        ...
        text_yields.write('\n \hline')
        print("Len x mass = {}, len m2sig = {}, len m2sigerr = {}, len m1sig = {}, len m1sigerr = {}, len exp ={}, len experr= {}, len obs = {}, len obsess = {}, len up1sig = {}, up1sigerr = {}, up2sig = {}, ipsigerr ={}".format(len(xmass),len(m2sig),len(m2sigerr),len(m1sig),len(m1sigerr),len(exp),len(experr),len(obs),len(obserr),len(up1sig),len(up1sigerr),len(up2sig),len(up2sigerr)))
...
if not options.blind:
    ...
    if ("tPrime" in cstr) :
      g_limit.GetXaxis().SetRangeUser(0.8, 3.0)
      g_limit.SetMinimum(5e-6) #0.005
      #g_limit.SetMinimum(8e-5) #0.005
      g_mclimit.SetMaximum(0.02)
      #g_limit.SetMaximum(0.001)
...
climits.SaveAs("hybrid_ionization/limits_combine_"+options.lumi.replace('.','p')+"fb_"+options.signals[options.signals.find('/')+1:options.signals.find('.')]+'_'+cstr+".pdf")

climits.SaveAs("hybrid_ionization/limits_combine_"+options.lumi.replace('.','p')+"fb_"+options.signals[options.signals.find('/')+1:options.signals.find('.')]+'_'+cstr+".C")

climits.SaveAs("hybrid_ionization/limits_combine_"+options.lumi.replace('.','p')+"fb_"+options.signals[options.signals.find('/')+1:options.signals.find('.')]+'_'+cstr+".root")
